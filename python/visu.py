...
if __name__ == "__main__":
    ...
    while cv2.getWindowProperty(vis.WINDOW_NAME, 0) >= 0:
        ...
        if focused_car is not None:
            ...
            cv2.putText(canvas, f"id={focused_car.id}", (5, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
            y += 33
        ...
        fps_text = f"FPS: {np.floor(1 / (time.time() - frame_start))}"
        cv2.putText(canvas, fps_text, (5, canvas.shape[0] - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
